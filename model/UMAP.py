...
def UMAPdecomposition(review_data, sentiment_data, index_data, vectorized_method):
    print('UMAPing...')
    ...
    for index, item in enumerate(new_review):
        if sentiment_data[index] == 1:
            ...
            counter_neg += 1
    df_neg.to_csv('../../dataset/' + vectorized_method + '_UMAP_neg_index_map.csv')
    df_pos.to_csv('../../dataset/' + vectorized_method + '_UMAP_pos_index_map.csv')
    fig = plt.figure()
    ax = Axes3D(fig)
    ...
    ax.scatter3D(np.array(negative_review)[:, 0], np.array(negative_review)[:, 1], np.array(negative_review)[:, 2],
                 label='negative')
    plt.legend()
    plt.savefig(vectorized_method + '_UMAP_visualization.png')
    np.save('Output/points/UMAP_pos_' + vectorized_method + '.npy', positive_review)
    np.save('Output/points/UMAP_neg_' + vectorized_method + '.npy', negative_review)
    plt.show()
